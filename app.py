...
# 🔥 مسار جديد مخصص لروابط الـ MPD عشان ما تخرب وتروح لمشغل App24 🔥
...
def proxy_mpd():
    ...
    try:
        r = requests.get(stream_url, headers=headers, timeout=15, verify=False)
        r.raise_for_status()
        content = r.text
        base_url = stream_url.rsplit('/', 1)[0] + '/'
        
        # حقن الرابط الأساسي عشان المشغل الخارجي يعرف يسحب الفيديو
    ...
    try:
        ...
        for line in lines:
            if not line.strip() or line.startswith("#"):
                ...
                if ".m3u8" in line.lower() or "playlist" in line.lower():
